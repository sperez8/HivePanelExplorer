...
<html>
    ...
    <body>
        ...
        <script src="c.elegans.herm_pharynx_1_nodes.js"></script>
        <script src="c.elegans.herm_pharynx_1_edges.js"></script>
        <script src="c.elegans.herm_pharynx_1_parameters.js"></script>
...
svg.append("text")
    ...
    .attr("font-size", "14px")
    .text('Axis Assignment') //add name of property used for node positionning, the rowtrait
...
svg.append("text")
    ...
    //.attr("font-weight","lighter")
    .attr("font-size", "14px")
    .text('Axis Position') //add name of property used for node positionning, the rowtrait
...
function plot(p){{
    ...
    cell.append("text")
        ...
        .attr("y", function (d) {{ return d.j-size/2 -padding/2 + 10}})
        .attr("text-anchor", "middle")
        .attr("font-family", FONT)
        .attr("font-weight","lighter")
        .attr("font-size", "14px")
    ...
    //creates axis labels
    //some parameters
    var outer_radius = radius.range()[1]+10
        x_shift = 10
        y_shift = 10
        stagger = 0
...
var node_tooltip = function (cx, cy, d, px, py, x, y){{
    if (py!=px){{
        text = '<b>' + d.cellname +'</b><br>'+ px + ': ' + round_value(x) + '<br>' + py + ': ' + round_value(y)
    }} else {{ text = '<b>' + d.cellname +'</b><br>'+ px + ': ' + round_value(x)}}
    tooltip   
